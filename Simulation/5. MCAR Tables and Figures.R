...
lapply(X = names, FUN = function(x) stat.mcar.5[[x]] %>% t %>% 
...
ggsave("Figures/MCAR/Rmse.pdf", dpi = 300, width = 20, height = 8, units = "cm")

## Bias means, variances, and correlation
ref <- stat.true %>% t %>% as.data.frame %>% .[,c(1:6,13:15)]

data <- lapply(X = names, FUN = function(x) stat.mcar.5[[x]] %>% t %>% 
...
ggsave("Figures/MCAR/Bias Descriptive Statistics.pdf", dpi = 300, width = 20, height = 22.5, units = "cm")

## Bias Beta's Sigma & R-squared 
ref <- analysis.true %>% t %>% as.data.frame() %>% .[,-c(3:4, 7:8)]

data <- lapply(X = names, FUN = function(x) analysis.mcar.5[[x]] %>% t %>% 
...
ggsave("Figures/MCAR/Bias Analysis.pdf", dpi = 300, width = 20, height = 15, units = "cm")

## Coverage Rate Betas
data.3 <- lapply(X = names, FUN = function(x) analysis.mcar.3[[x]] %>% t %>% 
                    .[,3:4] %>% as.data.frame %>% colMeans %>% stack() %>% mutate(method = x)) %>% 
   do.call("rbind", .) %>% 
   mutate(method = factor(method, levels = unique(method))) %>% mutate(type = ".3")
data.5 <- lapply(X = names, FUN = function(x) analysis.mcar.5[[x]] %>% t %>% 
                    .[,3:4] %>% as.data.frame %>% colMeans %>% stack() %>% mutate(method = x)) %>% 
   do.call("rbind", .) %>% 
   mutate(method = factor(method, levels = unique(method))) %>% mutate(type = ".5")
data.7 <- lapply(X = names, FUN = function(x) analysis.mcar.7[[x]] %>% t %>% 
                    .[,3:4] %>% as.data.frame %>% colMeans %>% stack() %>% mutate(method = x)) %>% 
   do.call("rbind", .) %>% 
...
ggsave("Figures/MCAR/Coverage Rate Betas.pdf", dpi = 300, width = 20, height = 9.5, units = "cm")

## Coverage Rate Means
data.3 <- lapply(X = names, FUN = function(x) stat.mcar.3[[x]] %>% t %>% 
                    .[,7:9] %>% as.data.frame %>% colMeans %>% stack() %>% mutate(method = x)) %>% 
   do.call("rbind", .) %>% 
   mutate(method = factor(method, levels = unique(method))) %>% mutate(type = ".3")

data.5 <- lapply(X = names, FUN = function(x) stat.mcar.5[[x]] %>% t %>% 
                    .[,7:9] %>% as.data.frame %>% colMeans %>% stack() %>% mutate(method = x)) %>% 
   do.call("rbind", .) %>% 
   mutate(method = factor(method, levels = unique(method))) %>% mutate(type = ".5")

data.7 <- lapply(X = names, FUN = function(x) stat.mcar.7[[x]] %>% t %>% 
                    .[,7:9] %>% as.data.frame %>% colMeans %>% stack() %>% mutate(method = x)) %>% 
   do.call("rbind", .) %>% 
   mutate(method = factor(method, levels = unique(method))) %>% mutate(type = ".7")
...
ggsave("Figures/MCAR/Coverage Rate Means.pdf", dpi = 300, width = 20, height = 9.5, units = "cm")
...
# RMSE
rbind(create_bias_table(stat.mcar.3, 0, 16:18, 0.3),
      create_bias_table(stat.mcar.5, 0, 16:18, 0.5),
      create_bias_table(stat.mcar.7, 0, 16:18, 0.7)) %>%
   .[,c("ind", "missing", names)] %>%
   write.csv(.,'Tables/MCAR/Rmse.csv')

# Bias Mu, var, cor
ref <- stat.true %>% t %>% as.data.frame %>% .[,c(1:6,13:15)]

rbind(create_bias_table(stat.mcar.3, ref, c(1:6,13:15), 0.3),
      create_bias_table(stat.mcar.5, ref, c(1:6,13:15), 0.5),
      create_bias_table(stat.mcar.7, ref, c(1:6,13:15), 0.7)) %>%
   .[,c("ind", "missing", names)] %>%
   write.csv(.,'Tables/MCAR/Bias Descriptive Statistics.csv')

# Bias Regression Analysis
ref <- analysis.true %>% t %>% as.data.frame() %>% .[,-c(3:4, 7:8)]

rbind(create_bias_table(analysis.mcar.3, ref, -c(3:4, 7:8), 0.3),
      create_bias_table(analysis.mcar.5, ref, -c(3:4, 7:8), 0.5),
      create_bias_table(analysis.mcar.7, ref, -c(3:4, 7:8), 0.7)) %>%
   .[,c("ind", "missing", names)] %>%
   write.csv(.,'Tables/MCAR/Bias Analysis.csv')

# Coverage Betas
rbind(create_coverage_table(analysis.mcar.3, 3:4, 0.3),
      create_coverage_table(analysis.mcar.5, 3:4, 0.5),
      create_coverage_table(analysis.mcar.7, 3:4, 0.7)) %>%
   .[,c("ind", "missing", names)] %>%
   write.csv(.,'Tables/MCAR/Coverage Rate Betas.csv')

# Coverage Mu 
rbind(create_coverage_table(stat.mcar.3, 7:9, 0.3),
      create_coverage_table(stat.mcar.5, 7:9, 0.5),
      create_coverage_table(stat.mcar.7, 7:9, 0.7)) %>%
   .[,c("ind", "missing", names)] %>%
   write.csv(.,'Tables/MCAR/Coverage Rate Means.csv')

# Average Confidence Interval Width Betas
rbind(create_confidence_width_table(analysis.mcar.3, 7:8, 0.3),
      create_confidence_width_table(analysis.mcar.5, 7:8, 0.5),
      create_confidence_width_table(analysis.mcar.7, 7:8, 0.7)) %>%
   .[,c("ind", "missing", names)] %>%
   write.csv(.,'Tables/MCAR/Average Confidence Interval Width Betas.csv')

# Average Confidence Interval Width Mu
rbind(create_confidence_width_table(stat.mcar.3, 19:21, 0.3),
      create_confidence_width_table(stat.mcar.5, 19:21, 0.5),
      create_confidence_width_table(stat.mcar.7, 19:21, 0.7)) %>%
   .[,c("ind", "missing", names)] %>%
   write.csv(.,'Tables/MCAR/Average Confidence Interval Width Mu.csv')