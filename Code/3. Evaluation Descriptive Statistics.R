...
statistics.imp <- function(x) {
   ...
   interval <- qt(.975, (n.rec-1)) * (vars / sqrt(nrow(mids)))
...
# apply to every MCAR simulation and to every method the statistics function
stat.mcar <- lapply(names, function(name) {
   map(imputed.mcar, `[`, c("data", name))} %>% vapply(., statistics.imp, format))
